...
model = Sequential()  #to start the alg
model.add(Convolution2D(nb_filters1, conv1_size, conv1_size, padding='same', input_shape=(img_width, img_height, 3)))
model.add(Activation("relu")) #pix waise it does the operation(reducing the size of img)
model.add(MaxPooling2D(pool_size=(pool_size, pool_size))) #agian conv reduce the img 
...
model.add(Dense(256)) #cateogrize
model.add(Activation("relu"))
model.add(Dropout(0.5)) #classifiying
model.add(Dense(tar, activation='softmax')) #7 dies including

model.compile(loss='categorical_crossentropy',
              optimizer=optimizers.RMSprop(learning_rate=0.0004), #optimizing to incnrease the acc
              metrics=['accuracy'])

hist=model.fit(train_tensors, train_targets ,validation_split=0.3, epochs=epoch, batch_size=10) #to store the train tensor and train targets 

show_history_graph(hist) #to display the graph
